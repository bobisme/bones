...
- When the last child of a goal reaches `done`, Bones emits a system event closing the goal: `actor: bones, reason: "all children complete"`.
...
- Partial parse: can extract timestamp/actor/type/item_id without touching JSON payload
...
Jaccard similarity on labels, dependencies, assignees, parent, and graph neighborhood proximity.
...
- Actor IDs: interned table, RLE-encoded references
...
Built-in MCP server exposing the full Bones API. Makes Bones instantly usable from any MCP-aware agent without CLI shelling.
...
| Sequential ID migration | Hash IDs from day one. |
