...
impl SearchView {
    ...
    pub fn tick(&mut self) -> Result<()> {
        if self.query != self.debounced_query && self.last_keystroke.elapsed() > Duration::from_millis(300) {
            self.perform_search()?;
    ...
    fn perform_search(&mut self) -> Result<()> {
        ...
        let effective_query = if !self.query.contains(' ') && !self.query.contains('*') && !self.query.contains(':') {
            format!("{}*", self.query)
        } else {
            self.query.clone()
        };

        let raw_results = hybrid_search(&effective_query, &conn, None, 20, 60)
        ...
        let items: Vec<ListItem> = self.results
            ...
            .map(|res| {
                let content = Line::from(vec![
                    Span::styled(format!("{:<10} ", res.item.item_id), Style::default().fg(Color::Cyan)),
                    Span::styled(format!("{:.4} ", res.item.score), Style::default().fg(Color::Green)),
                    Span::raw(&res.title),
                ]);
...
mod tests {
    ...
    fn search_view_performs_search() {
        ...
        let fts_count: i64 = conn.query_row("SELECT count(*) FROM items_fts", [], |r| r.get(0)).unwrap();
